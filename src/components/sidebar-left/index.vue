...
export default defineComponent({
  ...
  setup() {
    ...
    const icons = reactive(['chat', 'friend', 'group', 'bell'])
    // , 'contact' 先移除吧，后面再看这个页面展示问题
    const urls = reactive([
    ...
    const applyCount = computed(() => {
      let count = 0
      console.log('遍历state.friend.applys, 如果每一项item的applicant.id等于 user计算属性value.id 且状态是item.status为2')
      store.state.friend.applys.forEach(item => {
        if (item.applicant.id === user.value.id) {
          ...
          console.log('否则如果状态为0则直接加 未处理数量，这是拒绝了的吗')
          if (item.status === 0) {
            count += 1
          }
        }
    ...
    const clickHandle = (val) => {
      ...
      switch (val) {
        ...
        default:
          active.value = val
          console.log('通过计算属性召唤本尊进行修改，这一修改影响很大，会影响隔壁')
          break
      }
    }
