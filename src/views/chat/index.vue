      ...
      if (newVal.value) {
        ...
        switch (newVal.value.type) {
          ...
          case 1: // 聊天消息
            ...
            console.log('\n\ncase 1 聊天消息')
            if (friend.value
                && ((newVal.value.responseBody.message.to < 22800 && friend.value.id === newVal.value.responseBody.message.from)
                    || friend.value.id === newVal.value.responseBody.message.to)) {
              console.log('添加，滚动，设置已读一把梭')
              // 添加消息
